 ...
 *******************************************************/
/*
这是一个关键帧的类（KeyFrame），具体如下：

第1-13行代码为关键帧创建函数，主要包括创建关键帧时需要的各种信息和计算BRIEF点。

第15-31行代码为用于加载之前保存的关键帧信息的构造函数。

具体解释如下：

第2-6行代码定义了一个模板函数，用于缩小向量的大小。

第8-37行代码为关键帧创建函数，包括以下参数：

double _time_stamp: 时间戳。

int _index: 索引。

Vector3d &_vio_T_w_i: 相机在世界坐标系下的位姿。

Matrix3d &_vio_R_w_i: 相机在世界坐标系下的旋转矩阵。

cv::Mat &_image: 图像。

vector &_point_3d: 3D点在世界坐标系下的坐标。

vector &_point_2d_uv: 2D点在相机坐标系下的坐标。

vector &_point_2d_norm: 2D点在归一化相机坐标系下的坐标。

vector &_point_id: 3D点的ID。

int _sequence: 序列号。

第9行代码为关键帧的时间戳。此行代码设置了一个类变量 time_stamp。

第10行代码为关键帧的索引。此行代码设置了一个类变量 index。

第11-13行代码为相机的位姿。此行代码设置了类变量 vio_T_w_i 和 vio_R_w_i。其中，vio_T_w_i 表示相机在世界坐标系下的位姿，vio_R_w_i 表示相机在世界坐标系下的旋转矩阵。

第14行代码为计算BRIEF点。此行代码调用了 computeWindowBRIEFPoint() 和 computeBRIEFPoint() 函数。

第15行代码为关键帧的图像。此代码设置了一个类变量 image，并使用了 clone() 函数，将输入参数 _image 的副本存储到 image 类变量中。

第16行代码为缩略图。此代码设置了一个类变量 thumbnail，使用了 cv::resize() 函数将 image 类变量的图像大小缩小至（80，60）。

第17-21行代码为 3D 点信息。此代码设置了一个类变量 point_3d，其中存储了输入参数 _point_3d 中的所有 3D 点信息。

第22-26行代码为 2D 点在相机坐标系下的坐标。此代码设置了一个类变量 point_2d_uv，其中存储了输入参数 _point_2d_uv 中的所有 2D 点在相机坐标系下的坐标。

第27-31行代码为 2D 点在归一化相机坐标系下的坐标。此代码设置了一个类变量 point_2d_norm，其中存储了输入参数 _point_2d_norm 中的所有 2D 点在归一化相机坐标系下的坐标。

第32-33行代码为 3D 点的 ID。此代码设置了一个类变量 point_id，其中存储了输入参数 _point_id 中的所有 3D 点的 ID。

第34-35行代码为是否有环。此代码设置了一个类变量 has_loop，用于标记当前是否有环。

第36行代码为loop_index。此代码设置了一个类变量 loop_index，用于存储环的索引。

第37行代码为循环信息。此代码设置了一个类变量 loop_info，其中存储了环的循环信息。loop_info 是一个
